...
class ProductController extends AbstractController
{
    ...
    public function new(Request $request, CategoryRepository $categoryrepository): Response
    {
        ...
        $catname = $categoryrepository->findOneBy(['id' => 0]);
        $product = new Product();
    ...
    {
        $catlist = $categoryrepository->findAll();
        $catname = $categoryrepository->findOneBy(['id' => $product->getCategoryId()]);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->addFlash('success','Ürün Kayıt Güncelleme Başarılı');
    ...
    public function iupdate(Request $request,$id, Product $product): Response
    {
        ...
        try {
            $file->move(
                $this->getParameter('images_directory'),//servis.yamideki directoryi çağırdık
                $fileName
            );
